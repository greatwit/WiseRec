...
static sp<PersistentSurface> get_persistentSurface(JNIEnv* env, jobject object)
{
    ...
    return NULL;//android_media_MediaCodec_getPersistentInputSurface(env, object);
}
...
{
    GLOGW("setMaxFileSize(%lld)", (long long)max_filesize_bytes);
    sp<MediaRecorder> mr = getMediaRecorder(env, thiz);
...
android_media_MediaRecorder_prepare(JNIEnv *env, jobject thiz)
{
    ...
    if (surface != NULL) {
        ...
        GLOGI("prepare: surface=%p", native_surface.get());
