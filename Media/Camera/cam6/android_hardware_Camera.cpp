...
sp<Camera> get_native_camera(JNIEnv *env, jobject thiz, JNICameraContext** pContext)
{
    ...
    GLOGW("get_native_camera: context=%p, camera=%p", context, camera.get());
    if (camera == 0) {
        ...
        JNIEnv* env, Vector<jbyteArray>* buffers, size_t bufferSize)
{
    ...
    if (!buffers->isEmpty()) {
        GLOGW("Using callback buffer from queue of length %zu", buffers->size());
        ...
        if (obj != NULL) {
            ...
            if ((int)bufferLength < (int)bufferSize) {
                GLOGE("Callback buffer was too small! Expected %zu bytes, but got %d bytes!",
...
void JNICameraContext::copyAndPost(JNIEnv* env, const sp<IMemory>& dataPtr, int msgType)
{
    ...
    if (dataPtr != NULL) {
        ...
        GLOGW("copyAndPost: off=%zd, size=%zu", offset, size);
        ...
        if (heapBase != NULL) {
            ...
            if (msgType == CAMERA_MSG_RAW_IMAGE) {
                ...
                if (mCallbackBuffers.isEmpty()) {
                    GLOGW("Out of buffers, clearing callback!");
                ...
                GLOGW("Allocating callback buffer");
                obj = env->NewByteArray(size);
            }

            if (obj == NULL) {
                GLOGE("Couldn't allocate byte array for JPEG data");
                env->ExceptionClear();
            } else {
                env->SetByteArrayRegion(obj, 0, size, data);
            }
        } else {
            GLOGE("image heap is NULL");
        }
    }
                                ...
                                camera_frame_metadata_t *metadata)
{
    ...
    JNIEnv *env = AndroidRuntime::getJNIEnv();
    if (mCameraJObjectWeak == NULL) {
    	GLOGE("callback on dead camera object");
        return;
    }
    ...
    switch (dataMsgType) {
        ...
        default:
            GLOGW("dataCallback(%d, %p)", dataMsgType, dataPtr.get());
            copyAndPost(env, dataPtr, dataMsgType);
            break;
    }
...
void JNICameraContext::postMetadata(JNIEnv *env, int32_t msgType, camera_frame_metadata_t *metadata)
{
    ...
    obj = (jobjectArray) env->NewObjectArray(metadata->number_of_faces,
                                             mFaceClass, NULL);
    if (obj == NULL) {
    	GLOGE("Couldn't allocate face metadata array");
        return;
    }
...
{
    GLOGW("addCallbackBuffer: 0x%x", msgType);
    if (cbb != NULL) {
        Mutex::Autolock _l(mLock);
        switch (msgType) {
            case CAMERA_MSG_PREVIEW_FRAME: {
                ...
                GLOGW("Adding callback buffer to queue, %zu total",
...
void JNICameraContext::clearCallbackBuffers_l(JNIEnv *env, Vector<jbyteArray> *buffers) {
    GLOGW("Clearing callback buffers, %zu remained", buffers->size());
    ...
    jint cameraId, jobject info_obj)
{
    ...
    if (cameraId >= Camera::getNumberOfCameras() || cameraId < 0) {
    	GLOGE("%s: Unknown camera ID %d", __FUNCTION__, cameraId);
    ...
    jobject weak_this, jint cameraId, jint halVersion, jstring clientPackageName)
{
    ...
    if (clazz == NULL) {
    	...
    	GLOGE("Can't find android/hardware/Camera");
...
static void android_hardware_Camera_release(JNIEnv *env, jobject thiz)
{
    ...
    if (context != NULL) {
        camera = context->getCamera();
        context->release();
        GLOGW("native_release: context=%p camera=%p", context, camera.get());
...
{
    GLOGW("setPreviewCallbackSurface");
    ...
    if (camera->setPreviewCallbackTarget(gbp) != NO_ERROR) {
    	GLOGE("setPreviewCallbackTarget failed");
    }
}
...
{
    GLOGW("setHasPreviewCallback: installed:%d, manualBuffer:%d", (int)installed, (int)manualBuffer);
...
static void android_hardware_Camera_takePicture(JNIEnv *env, jobject thiz, jint msgType)
{
     ...
     */
    if (msgType & CAMERA_MSG_RAW_IMAGE) {
        GLOGW("Enable raw image callback buffer");
        if (!context->isRawImageCallbackBufferAvailable()) {
            GLOGW("Enable raw image notification, since no callback buffer exists");
            msgType &= ~CAMERA_MSG_RAW_IMAGE;
...
static jstring android_hardware_Camera_getParameters(JNIEnv *env, jobject thiz)
{
    ...
    if (params8.isEmpty()) {
        GLOGE("getParameters failed (empty parameters)");
        return 0;
    }
...
int register_android_hardware_Camera(JNIEnv *env)
{
    ...
    if (fields.point_constructor == NULL) {
        GLOGE("Can't find android/graphics/Point()");
        return -1;
    }
