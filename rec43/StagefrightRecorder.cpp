...
status_t StagefrightRecorder::setParamVideoCameraId(int32_t cameraId) {
    GLOGW("setParamVideoCameraId: %d", cameraId);
    if (cameraId < 0) {
        return BAD_VALUE;
    }
    mCameraId = cameraId;
...
status_t StagefrightRecorder::start() {
	...
	GLOGI("StagefrightRecorder mOutputFd %d", mOutputFd);
    // Get UID here for permission checking
    mClientUid = IPCThreadState::self()->getCallingUid();
	GLOGI("StagefrightRecorder mClientUid %d", mClientUid);
    if (mWriter != NULL) {
    ...
    switch (mOutputFormat) {
        ...
        case OUTPUT_FORMAT_MPEG_4:
			GLOGI("StagefrightRecorder OUTPUT_FORMAT_MPEG_4");
            status = startMPEG4Recording();
			GLOGI("StagefrightRecorder status %d", status);
            break;
...
status_t StagefrightRecorder::setupMediaSource(
    ...
    if (mVideoSource == VIDEO_SOURCE_DEFAULT
		...
		GLOGI("setupCameraSource 0");
        status_t err = setupCameraSource(&cameraSource);
		GLOGI("setupCameraSource err:%d", err);
        if (err != OK) {
            return err;
        }
        *mediaSource = cameraSource;
		...
		GLOGI("setupSurfaceMediaSource 0");
        status_t err = setupSurfaceMediaSource();
		GLOGI("setupSurfaceMediaSource 1");
        if (err != OK) {
            return err;
        }
...
status_t StagefrightRecorder::setupSurfaceMediaSource() {
    ...
    mIsMetaDataStoredInVideoBuffers = mSurfaceMediaSource->isMetaDataStoredInVideoBuffers();
...
status_t StagefrightRecorder::setupCameraSource(
	...
	GLOGI("setupCameraSource checkVideoEncoderCapabilities err:%d", err);
    Size videoSize;
    ...
    if (mCaptureTimeLapse) {
		...
		GLOGI("CameraSourceTimeLapse::CreateFromCamera 0");
        mCameraSourceTimeLapse = CameraSourceTimeLapse::CreateFromCamera(
                mCamera, mCameraProxy, mCameraId, mClientName, mClientUid,
                videoSize, mFrameRate, mPreviewSurface,
                mTimeBetweenTimeLapseFrameCaptureUs);
        *cameraSource = mCameraSourceTimeLapse;
		GLOGI("CameraSourceTimeLapse::CreateFromCamera 1");
    } else {
		GLOGI("CameraSource::CreateFromCamera 0 mCameraId:%d mClientUid:%d mClientName:%s", mCameraId, mClientUid, mClientName.string());
        *cameraSource = CameraSource::CreateFromCamera(
                mCamera, mCameraProxy, mCameraId, mClientName, mClientUid,
                videoSize, mFrameRate,
                mPreviewSurface, true /*storeMetaDataInVideoBuffers*/);
		GLOGI("CameraSource::CreateFromCamera 1");
    }
	if(mCamera==NULL)
		GLOGI("mCamera is null");
	GLOGI("mCamera.clear 0");
    mCamera.clear();
	GLOGI("mCamera.clear 1");
    mCameraProxy.clear();
	GLOGI("mCameraProxy.clear 1");
	
    if (*cameraSource == NULL) {
		GLOGI("cameraSource is null");
        return UNKNOWN_ERROR;
    }
	GLOGI("(*cameraSource)->initCheck() 0");
    if ((*cameraSource)->initCheck() != OK) {
		GLOGI("(*cameraSource).clear() 0");
        (*cameraSource).clear();
		GLOGI("(*cameraSource).clear() 1");
        *cameraSource = NULL;
        return NO_INIT;
    }
    ...
    if (mFrameRate == -1) {
        ...
        CHECK ((*cameraSource)->getFormat()->findInt32(
                    kKeyFrameRate, &frameRate));
        GLOGI("Frame rate is not explicitly set. Use the current frame "
             "rate (%d fps)", frameRate);
        mFrameRate = frameRate;
    }
	GLOGI("CHECK(mFrameRate != -1) 0");
    CHECK(mFrameRate != -1);
	GLOGI("CHECK(mFrameRate != -1) 1");
    mIsMetaDataStoredInVideoBuffers =
        (*cameraSource)->isMetaDataStoredInVideoBuffers();
	GLOGI("isMetaDataStoredInVideoBuffers");
    return OK;
}
...
status_t StagefrightRecorder::setupMPEG4Recording(
		...
		GLOGI("setupMPEG4Recording 1");
    mediaWriter->clear();
	GLOGI("mediaWriter->clear()");
    *totalBitRate = 0;
    status_t err = OK;
    sp<MediaWriter> writer = new MPEG4Writer(outputFd);
	GLOGI("new MPEG4Writer");
    if (mVideoSource < VIDEO_SOURCE_LIST_END) {

        sp<MediaSource> mediaSource;
		GLOGI("setupMediaSource 0");
        err = setupMediaSource(&mediaSource);
		GLOGI("setupMediaSource 1");
        if (err != OK) {
            return err;
        }
		...
		GLOGI("setupVideoEncoder");
        if (err != OK) {
            return err;
        }

        writer->addSource(encoder);
		GLOGI("writer->addSource");
    ...
    if (mMaxFileDurationUs != 0) {
        writer->setMaxFileDuration(mMaxFileDurationUs);
		GLOGI("setMaxFileDuration");
    }
    if (mMaxFileSizeBytes != 0) {
        writer->setMaxFileSize(mMaxFileSizeBytes);
		GLOGI("setMaxFileSize");
    }
    ...
    if (mStartTimeOffsetMs > 0) {
        reinterpret_cast<MPEG4Writer *>(writer.get())->
			...
			GLOGI("setStartTimeOffsetMs");
    }
	GLOGI("writer->setListener 0");
    writer->setListener(mListener);
	GLOGI("writer->setListener 1");
    *mediaWriter = writer;
    return OK;
}
...
status_t StagefrightRecorder::startMPEG4Recording() {
    int32_t totalBitRate;
	GLOGI("startMPEG4Recording 0");
    status_t err = setupMPEG4Recording(
            mOutputFd, mVideoWidth, mVideoHeight,
            mVideoBitRate, &totalBitRate, &mWriter);
	GLOGI("setupMPEG4Recording totalBitRate:%d err:%d\n", totalBitRate, err);
    if (err != OK) {
        return err;
    }
	...
	GLOGI("setupMPEG4MetaData t");
    err = mWriter->start(meta.get());
	GLOGI("setupMPEG4MetaData start err:%d", err);
    if (err != OK) {
        return err;
    }
